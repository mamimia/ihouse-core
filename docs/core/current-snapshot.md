...
| 128 | Conflict Center -- conflicts_router.py: GET /conflicts?property_id= ; cross-property tenant-scoped ACTIVE booking overlap detection; itertools.combinations per property; CRITICAL(>=3 nights)/WARNING(1-2); pair deduplication booking_a<booking_b; summary(total_conflicts/properties_affected/bookings_involved); JWT required; check_out exclusive; 39 tests | ✅ |

**3205 tests pass** (2 pre-existing SQLite skips, unrelated)
