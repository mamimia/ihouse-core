...
class TestGroupAListDlq:
    ...
    def test_a1_returns_200_with_entries(self):
        import json
        db = _db()
        r = _run(list_dlq_entries(tenant_id=TENANT, client=db))
    ...
    def test_a4_invalid_status_returns_400(self):
        r = _run(list_dlq_entries(tenant_id=TENANT, status="FLYING"))
        assert r.status_code == 400

    def test_a5_invalid_limit_returns_400(self):
        r = _run(list_dlq_entries(tenant_id=TENANT, limit=0))
        assert r.status_code == 400

    def test_a6_status_filter_propagated_in_response(self):
        import json
        db = _db([])
        r = _run(list_dlq_entries(tenant_id=TENANT, status="pending", client=db))
        body = json.loads(r.body)
    ...
    def test_a7_source_filter_propagated_in_response(self):
        import json
        db = _db([])
        r = _run(list_dlq_entries(tenant_id=TENANT, source="airbnb", client=db))
        body = json.loads(r.body)
...
class TestGroupBGetDlqEntry:
    ...
    def test_b1_returns_200_when_found(self):
        db = _db()
        r = _run(get_dlq_entry(envelope_id=ENVELOPE_ID, tenant_id=TENANT, client=db))
        assert r.status_code == 200, f"Got {r.status_code}: {r.body}"
    ...
    def test_b3_returns_404_when_not_found(self):
        db = _db([])
        r = _run(get_dlq_entry(envelope_id="ghost-env", tenant_id=TENANT, client=db))
