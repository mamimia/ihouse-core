"""
Phase 182 — Outbound Sync Auto-Trigger for BOOKING_CANCELED

Mirrors outbound_created_sync.py (Phase 176) for the BOOKING_CANCELED lifecycle event.

When BOOKING_CANCELED is APPLIED, this module fires the full outbound sync
...
@dataclass
class CanceledSyncResult:
    """Result of a single outbound sync attempt after BOOKING_CANCELED."""
    provider: str
    external_id: str
...
def _get_channels(property_id: str, tenant_id: str) -> list[dict]:
    ...
    try:
        ...
        logger.warning("outbound_canceled_sync: channel DB lookup failed: %s", exc)
        return []
...
def _get_registry() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning("outbound_canceled_sync: registry DB lookup failed: %s", exc)
        return {}


def fire_canceled_sync(
    *,
    booking_id: str,
    property_id: str,
    tenant_id: str,
...
) -> List[CanceledSyncResult]:
    """
    Fire the full outbound sync plan for a BOOKING_CANCELED event.
    ...
    Returns:
        List of CanceledSyncResult, one per action in the plan.
    ...
    if not resolved_channels:
        logger.debug(
            "outbound_canceled_sync: no channels for property=%s booking=%s — skipping",
    ...
    try:
        ...
        logger.warning(
            "outbound_canceled_sync: build_sync_plan failed for booking=%s: %s",
            booking_id, exc,
        )
        return []
    ...
    try:
        report = execute_sync_plan(
            ...
            tenant_id=tenant_id,
            actions=actions,
            event_type="BOOKING_CANCELED",  # Phase 185: route to .cancel() on adapters
        )
    except Exception as exc:
        logger.warning(
            "outbound_canceled_sync: execute_sync_plan failed for booking=%s: %s",
            booking_id, exc,
        )
        return []
