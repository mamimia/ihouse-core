...
class AirbnbAdapter(OTAAdapter):
    ...
    def normalize(self, payload: dict) -> NormalizedBookingEvent:
        """
        Normalize Airbnb webhook payload into canonical structure.

        Airbnb webhook fields:
          event_id       — unique event identifier
          reservation_id — Airbnb booking reference
          listing_id     — property identifier (Airbnb uses 'listing_id' not 'property_id')
    ...
    def to_canonical_envelope(
        ...
        if canonical_type == "BOOKING_AMENDED":
            # Deterministic booking_id — same rule as bookingcom and expedia
            booking_id = f"{self.provider}_{normalized.reservation_id}"

            # Extract provider-agnostic amendment fields (reads alteration.*)
