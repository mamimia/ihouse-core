...
# Note: You need to authenticate first using `uvx notebooklm-mcp-cli login`
# and identify your Notebook ID with `uvx notebooklm-mcp-cli notebooks list`
...
find docs -type f -name "*.md" | while read -r file; do
    echo "Uploading $file..."
    uvx notebooklm-mcp-cli source add "$NOTEBOOK_ID" "$file" || echo "Failed to add $file"
done
